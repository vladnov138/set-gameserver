...
class EnteringController extends Controller
{
    ...
    public function __invoke(Request $request)
    {
        ...
        $user_id = User::select('id')->where('api_token', $request['accessToken'])->get()[0]['id'];
        ...
        // return Score::select('players_scores')->where('id', $game_id)[0]['players_scores'];
        $scores = json_decode(Score::select('players_scores')->where('id', $game_id)->get()[0]['players_scores'], true);
        $scores[$user_id] = 0;
